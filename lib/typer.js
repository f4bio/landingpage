...
export function Typer(element, options, scenes) {
  ...
  const defaultOptions = { erase: 60, type: 80, break: 1000 };
  ...
  for (const key in defaultOptions) {
    ...
    if (!Object.prototype.hasOwnProperty.call(defaultOptions, key)) continue;
  ...
  function type() {
    ...
    // the next character
    state = scene.substring(0, state.length + 1);
    element.textContent = state;
  ...
  function play() {
    if (status === "ready") {
      ...
      // playing is a nonsense so that's
